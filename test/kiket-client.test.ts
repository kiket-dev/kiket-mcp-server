...
describe('KiketClient', () => {
  ...
  it('lists issues', async () => {
    ...
    mockInstance.get.mockResolvedValue({ data: { data: [issuePayload], meta: { current_page: 1, total_pages: 1, total_count: 1, per_page: 50 } } });
    await expect(client.listIssues({ status: 'todo' })).resolves.toEqual([issuePayload]);
    expect(mockInstance.get).toHaveBeenCalledWith('/api/v1/issues', { params: { status: 'todo' } });
  });

  it('creates issue and returns data', async () => {
    const client = new KiketClient({ baseUrl: 'https://api.example.com', apiKey: 'token', verifySSL: true });
    mockInstance.post.mockResolvedValue({ data: { issue: issuePayload } });
    await expect(client.createIssue({ title: 'Test', project_key: 'PRJ' })).resolves.toEqual(issuePayload);
  ...
  it('lists users via the regular API', async () => {
    const client = new KiketClient({ baseUrl: 'https://api.example.com', apiKey: 'token', verifySSL: true });
    const userPayload = {
      id: 7,
      name: 'Avery',
    ...
    mockInstance.get.mockResolvedValue({ data: [userPayload] });

    await expect(client.listUsers({ role: 'admin' })).resolves.toEqual([userPayload]);
    expect(mockInstance.get).toHaveBeenCalledWith('/api/v1/users', { params: { role: 'admin' } });
  });
});
